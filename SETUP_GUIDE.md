...
- `OWN_TEXT_MARKERS` _(опционально)_ — список характерных фраз ваших ручных сообщений в чате (отказы, формулы вежливости). Помогает парсеру правильно атрибутировать короткие ответы, которые вы пишете руками через UI HH. Шаблон с примерами — в `profile.example.py`.
...
Чтобы получать пуши на положительные сигналы и анкеты от автоботов:

```bash
export TELEGRAM_BOT_TOKEN=...   # @BotFather → /newbot
export TELEGRAM_CHAT_ID=...     # @userinfobot → ваш id
python3 notify.py               # проверка связи
```

Без этих переменных модуль `notify.py` работает как no-op — нотификации просто не отправляются, основной бот продолжает работать.
...
26 unit-тестов на детекторы (роли в чате, анкеты автоботов, отказы, положительные сигналы). Стандартная библиотека, дополнительные пакеты не нужны.

---

## Миграция истории чатов

Если вы только что обновились с версии до фикса парсера ролей по CSS-классам, прошлые ручные сообщения в `payload.history_tail` могут быть атрибутированы работодателю. Безопасный скрипт-миграция:

```bash
# 1. Бэкап БД (на всякий случай)
cp ~/.n8n-files/hh_metrics.sqlite ~/.n8n-files/hh_metrics.sqlite.bak

# 2. Dry-run — посмотреть, что будет переписано
python3 migrate_history_roles.py --days 60

# 3. Если ОК — записать в БД
python3 migrate_history_roles.py --days 60 --apply
```

Скрипт переписывает только префикс роли в блоках истории (`[Лидия] …` → `[Я] …`), исходный текст сообщений не трогает. Поддерживается также окно `--days 30 / 90 / 180`.
