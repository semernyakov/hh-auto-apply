...
| `HH_REPLY_COOLDOWN_SEC` | `21600` (6 ч) | Защита от карусели повторных Claude-вызовов: если последний входящий блок в чате не изменился со времени предыдущей обработки, Claude не вызывается. Дефолт рассчитан, чтобы покрывать ≥10 обходов при `HH_WATCH_INTERVAL_SEC=1800`. Когда работодатель напишет что-то новое — cooldown автоматически отпустит. `0` — отключить (не рекомендуется). |
